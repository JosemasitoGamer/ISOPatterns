...
void update(float dt,int gamepad) {
  if (remaining > 0)
    remaining -= dt;

  // Read the keyboard and move the player.
  if (standing && (gamepad & GAMEPAD_UP))//tigrKeyDown(screen, TK_SPACE))
    playerys -= 200;
...
int main(int argc, char* argv[]) {
  ...
  while (!tigrClosed(screen) && !tigrKeyDown(screen, TK_ESCAPE)) {
    ...
    if (tigrKeyDown(screen, TK_RETURN))
    {
      ...
      frame = 0;
      playerx = 160; playery = 200;
      if(isrecording) commands.clear();
    }
    unsigned gamepad = 0;
    if (isrecording)
    {
      ...
      //printf("gamepad value: %d\n", gamepad);

      commands.push_back(gamepad);

      unsigned command = gamepad;
    }
    else { //playing
      //gamepad = comand_list[frame]
